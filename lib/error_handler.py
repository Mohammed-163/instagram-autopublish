...
def create_fix_pr(gemini_client, github_pat: str, github_repo: str,
    ...
    import time
    import uuid
    # Unique per call (not just per-day): daily_generate.py / publish.py /
    # cleanup.py can each hit multiple failing rows in the same run, and a
    # shared "auto-fix/error-{date}" branch name would collide (second
    # branch-create call fails, second PR silently never opens).
    unique_suffix = f"{int(time.time())}-{uuid.uuid4().hex[:6]}"
    branch_name = f"auto-fix/error-{date_str}-{unique_suffix}"
    ...
    if branch_resp.status_code not in (200, 201):
        raise RuntimeError(f"Failed creating branch {branch_name}: {branch_resp.status_code} - {branch_resp.text}")
    ...
    file_info = file_resp.json() if file_resp.status_code == 200 else {}
    ...
    if update_resp.status_code not in (200, 201):
        raise RuntimeError(f"Failed committing fix to {file_path}: {update_resp.status_code} - {update_resp.text}")
