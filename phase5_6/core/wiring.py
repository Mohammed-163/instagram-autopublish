...
def wire_default_subscribers(bus: EventBus = default_event_bus) -> None:
    ...
    # ------------------------------------------------------------------ Weekly performance review (directly invoked by weekly_review.py)
    try:
        from engines.weekly_performance_review.engine import WeeklyPerformanceReviewEngine
        from lib.gemini_client import GeminiClient

        _container.register(
            "weekly_performance_review_engine",
            WeeklyPerformanceReviewEngine(
                metrics_repository=_container.resolve("metrics_repository"),
                hook_structure_repository=_container.resolve("hook_structures_repository"),
                topics_repository=_container.resolve("topics_repository"),
                weekly_planning_service=weekly_planning_service,
        ...
        _logging.getLogger("core.wiring").warning(
            "Weekly performance review engine not registered", exc_info=True
        )
