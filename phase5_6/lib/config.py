...
# IMAGE_VETTING_MODEL_NAME is now defined in the Gemini section above
...
# ---------------------------------------------------------------------------
# Gemini — FREE tier models only (no Pro, no paid models)
#
# There are FOUR independent model configuration variables:
#
...
#   ROTATION_MODELS         — comma-separated list consumed ONLY by the
#                             health-aware rotation engine in
#                             operational/gemini_rotation.py.
#
# These four variables are fully independent.  Changing ROTATION_MODELS
# does NOT change DEFAULT_TEXT_MODEL, IMAGE_VETTING_MODEL, or LEARNING_MODEL.
#
# Default values:
...
from operational.gemini_rotation import parse_model_rotation as _parse_model_rotation, FREE_MODELS as _FREE_MODELS  # noqa: E402

# Canonical free-tier model list (stable identifiers only — no aliases, no Pro,
# no preview, no image/video/audio/TTS/music/embedding/robotics models).
GEMINI_FREE_MODELS: list = list(_FREE_MODELS)

# ---------------------------------------------------------------------------
# ROTATION_MODELS — controls the rotation engine ONLY
# ---------------------------------------------------------------------------
# Configurable rotation order for the health-aware GeminiRotationEngine.
# Override at runtime by setting ROTATION_MODELS (or the legacy alias
# GEMINI_MODEL_ROTATION) — no code change required.
#
# This list defines the fallback/tiebreaker order when pairs are equally
# healthy.  It does NOT influence which model is used for text generation,
# image vetting, or learning tasks.
_rotation_raw = (
    _os.environ.get("ROTATION_MODELS", "")
    or _os.environ.get("GEMINI_MODEL_ROTATION", "")
)
ROTATION_MODELS: list = _parse_model_rotation(_rotation_raw if _rotation_raw else None)

# Legacy alias — kept for backwards compatibility with existing callers.
GEMINI_MODEL_ROTATION: list = ROTATION_MODELS

# ---------------------------------------------------------------------------
# DEFAULT_TEXT_MODEL — general text generation
# ---------------------------------------------------------------------------
# Must be set independently; NEVER derived from ROTATION_MODELS.
DEFAULT_TEXT_MODEL: str = _os.environ.get("DEFAULT_TEXT_MODEL", "gemini-3.5-flash-lite")

# Legacy alias — kept so existing callers that reference GEMINI_MODEL_NAME
# continue to work without modification.
GEMINI_MODEL_NAME: str = DEFAULT_TEXT_MODEL

# ---------------------------------------------------------------------------
# IMAGE_VETTING_MODEL — image selection / moderation
# ---------------------------------------------------------------------------
# Must be set independently; NEVER derived from ROTATION_MODELS position.
IMAGE_VETTING_MODEL: str = _os.environ.get("IMAGE_VETTING_MODEL", "gemini-3.5-flash-lite")

# Legacy alias — kept so existing callers that reference IMAGE_VETTING_MODEL_NAME
# continue to work without modification.
IMAGE_VETTING_MODEL_NAME: str = IMAGE_VETTING_MODEL

# ---------------------------------------------------------------------------
# LEARNING_MODEL — learning layer / analysis tasks
# ---------------------------------------------------------------------------
LEARNING_MODEL: str = _os.environ.get("LEARNING_MODEL", "gemini-3.5-flash")
