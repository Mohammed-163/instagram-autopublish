...
def _wire_bridges(p8_container, p9_container, p10_app):
    ...
    try:
        ...
        logger.info("Phase7→Phase8 publishers: %s → %s", type(p7.in_process_publisher).__name__, getattr(p8_run, "__module__", type(p8_run).__name__))
...
def _run_event_flow_test(components):
    ...
    p10 = components["phase10_app"]

    def on_coverage(event):
        if isinstance(event, KnowledgeCoverageCalculated):
            logger.warning("[EVENT-TRACE] Phase 9 received and published KnowledgeCoverageCalculated")

    p9.publisher.subscribe(on_coverage)
    try:
        p10.publisher.subscribe("opportunity.discovered", lambda event: logger.warning(
            "[EVENT-TRACE] Phase 10 received coverage update"
        ))
    except TypeError:
        p10.publisher.subscribe("*", lambda event: logger.warning(
            "[EVENT-TRACE] Phase 10 received coverage update"
        ))
...
if __name__ == "__main__":
    ...
    try:
        ...
        import traceback
        logger.exception("Unified AI pipeline bootstrap failed")
        traceback.print_exc()
        sys.exit(1)
